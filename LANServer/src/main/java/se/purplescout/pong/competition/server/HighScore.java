...
public class HighScore extends GridPane {
    ...
    public void setFights(List<AutoFight> fights) {
        ...
        if (fights.isEmpty()) {
            ...
            SortedMap<Paddle, Integer> scores = HighScoreUtil.calculateAndSortScoresForEachPaddle(fights);
    ...
    private Label buildTeamNameLabel(Font font, int place, Map.Entry<Paddle, Integer> entry) {
        ...
        if (place == 1) {
            name.setGraphic(new ImageView("se/purplescout/pong/competition/server/medal_gold.png"));
        } else if (place == 2) {
            name.setGraphic(new ImageView("se/purplescout/pong/competition/server/medal_silver.png"));
        } else if (place == 3) {
            name.setGraphic(new ImageView("se/purplescout/pong/competition/server/medal_bronze.png"));
        }
    ...
    private Region buildFightDetailsFor(Paddle pov, OnPaddleRemovedListener onPaddleRemovedListener) {
        ...
        Label removeButton = new Label(null, new ImageView("se/purplescout/pong/competition/server/delete.png"));
